...
```{r join-testing-data}
# Filter School info to just district schools
...
```{r get-sch-codes, message=FALSE}
sch_data <- read_csv("UD_tblSSSI_20240503_130132.csv")
...
data_final |> 
  ...
  ungroup() |> 
  ggplot(aes(x = value)) +
  geom_density(aes(fill = BIPOC), 
               alpha = 0.5) +
  scale_fill_manual(values = c('skyblue3', 'orange2')) +
  facet_wrap(~fct_reorder(variable, avg), 
             ncol = 1, 
             scales = 'free_y') +
  geom_vline(aes(xintercept = avg), 
             color = 'red3', 
